...
class DeBiasedModel(lightning.LightningModule):
...
class PostHocIRLSModelV2(lightning.LightningModule):
    def __init__(self, model, train_dataloader, num_covars = None, pen_factor = 1e-6, orthogonalize=False, max_iters=20, tol=1e-3):
        """
        Applies IRLS to re-estimate last layer parameters of a pre-trained model over
        the training data set for generalized linear models with responses from the exponential family.
        """
        ...
        # Initialize parameters for orthogonalization.
        self.model.ortho_parameters = torch.nn.Parameter(
            torch.zeros((self.model.num_covars, self.model.num_features)), requires_grad=False
        )
    ...
    def _irls_orth_estimation(self, data_loader):
        htwh_orth = torch.zeros((self.model.num_features, self.model.num_features), device=self.model.device)
        htwx_orth = torch.zeros((self.model.num_features, self.model.num_covars), device=self.model.device)
        for batch in data_loader:
            u = batch["image"].to(self.model.device)
            x = batch["covar"].to(self.model.device)
        ...
        return torch.linalg.lstsq(htwh_orth, htwx_orth).solution

    def _irls_estimation(self, data_loader):
        """Perform IRLS to re-estimate the weights of struct_predictor and deep_predictor."""
        #total_residual = self.tol
        struct_weights_old = self.tol * torch.ones((self.model.num_covars, 1), device=self.model.device)
        deep_weights_old = self.tol * torch.ones((self.model.num_features, 1), device=self.model.device)
        ...
        for iter in range(self.max_iters):
            ...
            #total_residual_new = 0

            # Get projection parameters.
            irls_ortho_params = self._irls_orth_estimation(data_loader)

            # Get weighted LS matrices.
            with torch.no_grad():
                for batch in data_loader:
                    u = batch["image"].to(self.model.device)
                    x = batch["covar"].to(self.model.device)
                    ...
                    # Project x onto the orthogonalized space wrt. h.
                    x_ortho = x - h @ irls_ortho_params

                    # Update weighted matrices
                    xtwx += x_ortho.T @ W @ x_ortho
                    xtwz += x_ortho.T @ W @ z
                    ...
                    #total_residual_new += torch.sum(torch.abs(y - mu)).item()

            # Solve for struct_predictor weights, guaranteeing invertibility.
            struct_weights = torch.linalg.solve(xtwx, xtwz)
            self.model.struct_predictor.weight.data = struct_weights.T

            # Solve for deep_predictor weights using the ridge penalty.
            ridge_penality_htwh = htwh + self.pen_factor * torch.eye(self.model.num_features, device=self.model.device)
            deep_weights = torch.linalg.lstsq(ridge_penality_htwh, htwz).solution
            self.model.deep_predictor.weight.data = deep_weights.T + self.model.struct_predictor.weight @ irls_ortho_params.T

            # Convergence check
            #total_residual_new = total_residual_new/len(data_loader)
            #print(abs(total_residual_new - total_residual)/total_residual)
            #if abs(total_residual_new - total_residual)/total_residual < self.tol:
            #    break
            #else:
            #    total_residual = total_residual_new

            # Check wether weights have converged.
            d_struct = torch.norm(self.model.struct_predictor.weight.data - struct_weights_old)/torch.norm(struct_weights_old)
            d_deep = torch.norm(self.model.deep_predictor.weight.data - deep_weights_old)/torch.norm(deep_weights_old)
            if d_struct < self.tol and d_deep < self.tol:
                break
            else:
                print("Struct: ", d_struct, "Deep: ", d_deep)
                struct_weights_old = self.model.struct_predictor.weight.data
                deep_weights_old = self.model.deep_predictor.weight.data
