...
c1 <- make_plot(
  df_bz %>% filter(
    effect == "fr",
    metric == "bias2",
    model %in% c("DNN w. Controls + Orth.", "DNN + Regress Out [34]")
  ),
...
ggsave("graphics/Fig1b_Pfeuffer.pdf", b, width = 4.16, height = 2.8, units = "in", dpi=600, device = cairo_pdf)
ggsave("graphics/Fig1c_Pfeuffer.pdf", c, width = 4.16, height = 2.8, units = "in", dpi=600, device = cairo_pdf)
